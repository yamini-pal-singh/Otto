...
Test suite for **CSR** stage of Otto, covering API (pytest) and ASR/STT tests.
...
| **Audio URLs** | 10 MP3 recordings in `tests/api/call_processing_data.py` → `STAGING_AUDIO_URLS` |
...
To **install** this data into the Otto backend (upload SOP + submit audio recordings for processing):
...
Requires `.env` with `OTTO_API_KEY`. The script uploads the Intake Calls SOP via URL and submits all staging audio URLs; jobs run asynchronously on the server.
